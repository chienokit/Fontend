...
import {CLOSED, IN_PROGRESS, OPEN, OVER_DUE, RECEIVED, TOTAL} from '@shared/constants/system-report.constant';
...
import {AdvancedSearchReportComponent} from '@component/report/system-report/advanced-search-report/advanced-search-report.component';
...
export class SystemReportComponent implements OnInit {
          ...
          if (!!buildings) {
            ...
            this.loadDataChart(
              ...
              this.endDate
                ...
                : undefined, this.filterRequest.ticketType
            );
          }
        }
      });
  ...
  loadDataChart(buildingIds: string[], startAt?: number, endAt?: number, ticketType?: string): void {
    this.ticketService
  ...
  getDetailReport(status: string, item: ITicketReport, action?: string): void {
    ...
    if (!!action) {
      if (action === OVER_DUE) {
        dataObject.queryParams.expectedStartAt = CommonUtil.getStartOfDay(new Date().getTime());
      } else {
        // trừ đi 1 phút để TH: expectedEndAt cùng ngày với ngày dự kiến hoàn thành thì không tính là hết hạn
        dataObject.queryParams.expectedEndAt = CommonUtil.getEndOfDay(new Date().getTime()) - 60000;
      }
    }
