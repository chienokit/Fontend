...
import { USER_ACTIVE, USER_EMPLOYEE, USER_INACTIVE, USER_PROFILE_INTERNAL, USER_PROFILE_LDAP } from '@shared/constants/user.constant';
...
export class UserComponent implements OnInit {
  ...
  loadData(pageNumber: number, size: number, sortBy?: string, isLoading = true): void {
    ...
    this.userService.search(this.userRequest, isLoading = true).subscribe((response: any) => {
  ...
  openChangePassword(user: User): void {
    const base = CommonUtil.modalBase(ChangePasswordComponent, {
      user,
    }, '30%');
    const modal: NzModalRef = this.modalService.create(base);
    modal.afterClose.subscribe(result => {
  ...
  getTranslate(s: string): string {
    return this.translateService.instant(this.pathTranslateAccountType + '' + s);
  }
  ...
  getAccountTypeTrans(user: User): string {
    if (user?.accountType) {
      if (this.getTranslate(user?.accountType?.toLowerCase()) === this.pathTranslateAccountType) {
  ...
  openAdvancedSearch(): void {
    const base = CommonUtil.modalBase(AdvancedSearchUserComponent, {
      users: this.users,
