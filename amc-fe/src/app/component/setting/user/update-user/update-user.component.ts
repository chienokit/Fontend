...
export class UpdateUserComponent implements OnInit {
  ...
  initData(): void {
    this.getDataParam();
    this.searchBuilding(this.keyword, true);
    this.searchCompany(this.keyword, true);
    this.findId();
  ...
  getDataParam(): void {
    ...
    if (this.contactObject?.organizationId) {
      this.searchBuildingsByOrganizationId(this.contactObject?.organizationId, true);
    }
    this.initForm();
    this.clearValid();
  }
  ...
  searchBuilding(keyword: string, isLoading = false): void {
  ...
  searchRoles(keyword: string, isLoading = false): void {
    const options = {
      keyword: keyword.trim(),
      userLevel: this.userLevelRole.trim(),
    };
    this.roleService.searchAutoComplete(options, isLoading).subscribe((res: any) => {
      this.roles = res.body?.data;
    });
  }

  // Lấy thông tin company fill ra ô select company **/
  searchCompany(keyword: string, isLoading = false): void {
