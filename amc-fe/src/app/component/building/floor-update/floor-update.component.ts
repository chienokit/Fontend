...
export class FloorUpdateComponent implements OnInit {
  ...
  initForm(): void {
    this.form = this.fb.group({
      name: [{value: this.isUpdate ? this.floor?.name : null, disabled: false},
      ...
      totalArea: [{value: this.isUpdate ? this.floor?.totalArea : null, disabled: false},
        [Validators.required, Validators.min(1), Validators.max(this.building?.area || BUILDING_VALIDATOR.area.max)]],
      // availableArea: [{value: this.isUpdate ? this.floor?.availableArea : null, disabled: true},
      //   [Validators.required, Validators.min(1)]],
      floorNumber: [{value: this.isUpdate ? this.floor?.floorNumber : null, disabled: this.isUpdate},
        [Validators.required, Validators.min(-18), Validators.max(this.building?.totalFloor || BUILDING_VALIDATOR.area.max)]],
  ...
  onSubmit(): void {
    ...
    if (this.form?.value?.totalArea) {
      this.form.get('totalArea')?.setValue(Number(this.form.value?.totalArea || 0));
    }
    if (this.form?.value?.floorNumber) {
      this.form.get('floorNumber')?.setValue(Number(this.form.value?.floorNumber || 0));
    }
    const floor: Floor = {
      ...this.form.value,
    };
    ...
    this.buildingService.createFloor(this.buildingId, body, true).subscribe(res => {
  ...
  onUpdateSubmit(): void {
    ...
    const body = CommonUtil.trim(floor);
    this.buildingService.updateFloor(body, this.buildingId, this.floor.id, true).subscribe(res => {
      if (res.status === STATUS.SUCCESS_200) {
  ...
  delete(): void {
    ...
    modal.afterClose.subscribe(result => {
      if (result?.success) {
        this.buildingService.deleteFloor(this.buildingId, this.floor.id).subscribe(res => {
          if (res.status === STATUS.SUCCESS_200) {
