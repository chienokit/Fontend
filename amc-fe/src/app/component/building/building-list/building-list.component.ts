...
export class BuildingListComponent implements OnInit {
  ...
  loadData(pageIndex: number, pageSize: number, sortBy?: string, isLoading = true): void {
    ...
    this.buildingService.search(this.buildingRequest, isLoading).subscribe(res => {
  ...
  hasUserLevel(): boolean {
    ...
    return !!myProfile?.userLevel && (USER_LEVEL_CENTER.includes(myProfile?.userLevel || '') || !!myProfile.userPrimary?.isRoot);
  }
  ...
  openConfirmInvalid(building: IBuilding): void {
    if (!!building.id) {
      if (building.status === BUILDING_STATUS.ACTIVE.value) {
        ...
        this.buildingGroupModal.content = this.translateService.instant('model.building.confirmContent',
          {buildingCode: building.code});
        this.buildingGroupModal.callBack = () => {
          this.buildingService.invalidBuilding(building.id || '').subscribe(res => {
        ...
        this.buildingGroupModal.content = this.translateService.instant('model.building.confirmActive',
          {buildingCode: building.code});
        this.buildingGroupModal.callBack = () => {
          this.buildingService.activeBuilding(building.id || '').subscribe(res => {
            if (res?.status === STATUS.SUCCESS_200) {
              this.loadData(this.pageIndex, this.pageSize);
            }
          });
