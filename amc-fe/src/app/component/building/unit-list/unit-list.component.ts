...
import {
  LEASING_STATUS, LEASING_STATUS_LIST, UNIT_STATUS,
  UNIT_TYPE,
  UNIT_TYPE_LIST
...
export class UnitListComponent implements OnInit {
  ...
  getIndex(index: number): number {
    return CommonUtil.getIndex(index, this.unitRequest.pageIndex, this.unitRequest.pageSize);
  }
  ...
  loadData(isLoading = true): void {
    const unitSearchRequest = {
      ...
      leasingStatus: this.unitRequest.leasingStatus.filter(item => item !== LEASING_STATUS.DEFAULT.value),
      ...
      isDefault: !!this.unitRequest.leasingStatus.find(item => item ===  LEASING_STATUS.DEFAULT.value)
    };
  ...
  getUnitStatus(status?: string): string {
    if (!!status) {
      const unitStatus = this.leasingStatusList.find(item => item.value === status)?.label || 'unknown';
  ...
  getType(type?: string): string {
    if (!!type) {
      const unitType = this.unitTypeList.find(item => item.value === type)?.label || 'unknown';
  ...
  format(value: IUnit, type: string): { text: string, sClass: string } {
    ...
    if (type === 'leasingStatus') {
      if (!!value[type]) {
        text = this.translateService.instant([this.translatepath + 'uStatus', value[type]?.toLowerCase()].join('.'));
        sClass = this.leasingStatusList.find(item => item.value === value[type])?.class || 'badge-secondary';
      } else if (value.isDefault) {
        text = this.translateService.instant(this.translatepath + 'uStatus.default');
  ...
  openConfirmDelete(unit: IUnit): void {
    this.unitGroupModal.content = this.translateService.instant('model.unit.confirmDeleteUnit', {code: unit.code});
    this.unitGroupModal.callBack = () => {
      this.unitService.removeUnit(unit.id).subscribe(res => {
        if (res?.status === STATUS.SUCCESS_200) {
          this.loadData();
          this.toastService.success(this.translateService.instant('model.unit.deleteUnitSuccess'));
        }
      });
    };
    this.isVisible = true;
  ...
  filter(): void {
    const base = CommonUtil.modalBase(UnitFilterComponent, {
