...
export class UnitDetailComponent implements OnInit {
  ...
  initForm(): void {
    this.form = this.fb.group({
      buildingId: [{value: this.unitDetail?.buildingId || null, disabled: (this.isDetail || this.isUpdate)},
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.ID_MAX_LENGTH.MAX)]],
      floorId: [{value: this.unitDetail?.floorId || null, disabled: true},
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.ID_MAX_LENGTH.MAX)]],
      price: [{value: this.unitDetail?.price || 0, disabled: this.isDetail},
        [Validators.min(0)]],
      area: [{value: this.unitDetail?.area || null, disabled: this.isDetail},
      ...
      leasingStatus: [{
        ...
        [Validators.required]],
      note: [{value: this.unitDetail?.note || null, disabled: this.isDetail},
        [Validators.maxLength(LENGTH_VALIDATOR.NOTE_MAX_LENGTH.MAX)]],
      type: [{value: this.unitDetail?.type || UNIT_TYPE.OFFICE.value, disabled: this.isDetail},
        [Validators.required]],
    });
  }
  ...
  onCreate(): void {
    ...
    if (!!this.form.get('price')) {
      this.form.get('price')?.setValue(CommonUtil.formatToNumber(this.form.value.price || 0));
    }

    if (this.form.invalid) {
      this.toastService.error(this.translateService.instant('model.unit.error.formInvalid'));
      return;
    }
    this.unitService.create(this.form.value, true).subscribe(
      (res) => {
        if (res.status === STATUS.SUCCESS_200) {
          this.toastService.success(this.translateService.instant(this.pathTranslate + 'success.create'));
          this.router.navigate(['/building/unit']);
        } else {
          this.toastService.error(this.translateService.instant(this.pathTranslate + 'create-fail'));
        }
      }
    );
  }
  ...
  loadDataBuilding(keyword?: string, buildingIds?: string[]): void {
    ...
    this.buildingService.searchBuildingAutoComplete(buildingSearchBuildingRequest).subscribe(
      (data: any) => {
  ...
  getUnitById(id: string): void {
    this.unitService.getById(id).subscribe(
      (res) => {
        ...
        if (this.unitDetail.leasingStatus !== this.leasingStatus.AVAILABLE.value && this.isUpdate) {
          window.history.back();
        }
  ...
  searchFloor(keyword?: string): void {
    if (!this.form.get('buildingId')?.value) {
      this.toastService.error(this.pathTranslate + 'error.requiredSelectBuilding');
      this.form.get('floorId')?.disable();
      return;
    }
    this.buildingService.simpleSearchFloor(this.form.get('buildingId')?.value, {
      keyword, minArea: (this.isDetail || this.isUpdate) ? undefined : 1
    }).subscribe(
      res => {
        // show data floor with building
        this.floorList = res.body?.data as Array<IFloor>;
        if (!!this.unitDetail.floorId) {
          this.floorSelected = this.floorList.find(item => item.id === this.unitDetail.floorId);
          if (this.isUpdate) {
            this.maxAreaAvailable = Number(((this.floorSelected?.availableArea! +
              +this.unitDetail?.area!) || BUILDING_VALIDATOR.area.max)?.toFixed(2));
            this.form.controls.area.setValidators([Validators.required,
              Validators.max(Number((this.floorSelected?.availableArea! +
                this.unitDetail?.area!)?.toFixed(2)) || BUILDING_VALIDATOR.area.max),
              Validators.min(1)]);
            this.form.controls.area.updateValueAndValidity();
          } else {
            this.maxAreaAvailable = Number((this.floorSelected?.totalArea || BUILDING_VALIDATOR.area.max)?.toFixed(2));
            this.form.controls.area.setValidators([Validators.required,
              Validators.max(Number(this.floorSelected?.totalArea?.toFixed(2)) || BUILDING_VALIDATOR.area.max), Validators.min(1)]);
            this.form.controls.area.updateValueAndValidity();
          }
        }
      }
  ...
  public selectFloor(floorId: string): void {
    if (!!this.form.get('floorId')?.value) {
      ...
      if (!!this.floorSelected?.availableArea || this.floorSelected?.availableArea === 0) {
        if (this.floorSelected.availableArea === 0) {
          this.form.get('floorId')?.setValue(null);
        }
        this.form.controls.area.setValidators([Validators.required, Validators.max(Number(this.floorSelected.availableArea?.toFixed(2))),
          Validators.min(1)]);
        this.form.controls.area.updateValueAndValidity();
        this.maxAreaAvailable = Number(this.floorSelected?.availableArea?.toFixed(2)) || BUILDING_VALIDATOR.area.max;
        if (this.form.get('area')?.value > this.floorSelected.availableArea) {
          this.toastService.warning(this.translateService.instant(this.pathTranslate + 'error.notEnoughArea',
  ...
  onUpdate(): void {
    ...
    if (this.form.invalid) {
      this.toastService.error(this.translateService.instant('error.formInvalid'));
      return;
    }
    if (!this.unitId) {
      return;
    }
    this.unitService.update(this.form.value, this.unitId, true).subscribe(
      (res) => {
        if (res.body?.code === STATUS.SUCCESS_200) {
          this.toastService.success(this.translateService.instant(this.pathTranslate + 'success.msg'));
          window.history.back();
        }
      }
    );
  ...
  format(): { text: string, sClass: string } {
    ...
    if (this.unitDetail) {
      const leasing = this.unitStatusList.find(l => l.value === this.unitDetail.leasingStatus);
      text = this.translateService.instant(leasing?.label || 'model.unit.uStatus.available');
