...
import { ACTION_TYPE, BUILDING_STATUS, BUILDING_VALIDATOR } from '@shared/constants/building.constants';
...
export class BuildingDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.isCreate) {
      this.authService.hasPermissionAccess(['building:create'], [USER_LEVEL_CENTER]);
    }
    this.initForm();
    // this.loadDataFloor();
  }
  ...
  initForm(): void {
    this.form = this.fb.group({
      code: [{value: this.building?.code || '', disabled: this.isUpdate},
        [Validators.required, Validators.pattern(VALIDATORS.CODE), Validators.maxLength(LENGTH_VALIDATOR.CODE_MAX_LENGTH.MAX)]],
      name: [this.building?.name || '',
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.NAME_MAX_LENGTH.MAX)]],
      totalFloor: [{value: this.building?.totalFloor || '', disabled: false},
        [Validators.required, Validators.min(1), Validators.max(200)]],
      address: [this.building?.address || '',
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.ADDRESS_MAX_LENGTH.MAX)]],
      note: [{value: this.building?.note || null, disabled: false},
        [Validators.maxLength(LENGTH_VALIDATOR.NOTE_MAX_LENGTH.MAX)]],
      status: [this.building?.status || this.buildingStatus.ACTIVE.value,
        [Validators.required]],
      deleted: [false],
      area: [this.building?.area || null,
        [Validators.required, Validators.min(1), Validators.max(BUILDING_VALIDATOR.area.max )]],
    });
  ...
  findBuildingId(id: string): void {
    if (!!id) {
      this.buildingService.getById(id).subscribe(response => {
        ...
        this.buildingId = id;
        this.isUpdate = !!(this.buildingId && window.location.href.includes('detail'));
  ...
  loadDataFloor(isLoading = true): void {
    ...
    this.buildingService.searchFloorByBuilding(id, this.floorQuery, isLoading).subscribe(response => {
  ...
  loadManager(keyword?: string, isLoading = true): void {
    ...
    this.buildingService.searchManagerByBuilding(this.buildingId, this.managerQuery, isLoading).subscribe(response => {
  ...
  onCreateBuilding(): void {
    ...
    if (!!this.form.get('totalFloor')) {
      this.form.get('totalFloor')?.setValue(Number(this.form.value.totalFloor || 0));
    }
  ...
  onUpdateSubmit(): void {
    ...
    const body = CommonUtil.trim(building);
    if (!this.building?.id) {
      this.toast.error(this.translateService.instant('model.building.error.buildingId'));
      return;
    }
    this.buildingService.update(body, this.building.id, true).subscribe(res => {
      if (res.status === STATUS.SUCCESS_200) {
  ...
  createFloor(): void {
    const base = CommonUtil.modalBase(FloorUpdateComponent, {
      isUpdate: false,
  ...
  openConfirm(actionType: string): void {
    if (actionType === ACTION_TYPE.CREATE_BUILDING) {
      ...
      this.groupConfirmBuilding.title = 'model.building.managerBuilding.message.create';
  ...
  public getTranslateManager(key: string): string {
    return this.translateService.instant(`model.building.managerBuilding.${key}`);
  }
  ...
  redirectToUpdateAccount(userid?: string, accountType?: string): void {
    ...
    if (userid && accountType) {
      ...
      this.route.navigate([`setting/user/${userid}/update`], {queryParams: {typeUser: type}});
    }
  }
  ...
  deleteFloor(floor: IFloor): void {
    this.buildingService.deleteFloor(this.buildingId, floor.id).subscribe(res => {
      if (res.status === STATUS.SUCCESS_200) {
        this.toast.success(this.translateService.instant('model.floor.success.delete'));
        this.loadDataFloor();
      }
    });
    this.isVisible = true;
  ...
  hasUserLevel(): boolean {
    ...
    if (USER_LEVEL_CENTER.includes(myProfile?.userLevel || '') || !!myProfile.userPrimary?.isRoot) {
      return true;
    }
    return false;
  }
  ...
  redirectToUpdateBuilding(buildingId: string): void {
    this.route.navigate([`building/${buildingId}/detail`], {queryParams: {tab: 'floor'}});
  }
