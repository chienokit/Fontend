...
import { ComplaintFilterComponent } from '@component/complaint/complaint-list/complaint-filter/complaint-filter.component';
  ...
  handleAction(result: { success: boolean }): void {
    if (result && result?.success) {
      ...
      if(this.isCreateTicket && this.complaintSelected.id) {
        ...
        this.route.navigate([`/ticket/create-by-complaint/${this.complaintSelected.id}`]);
      }
    } else {
      this.isVisible = false;
    }
  ...
  openAdvancedSearch(
    ...
    modal.afterClose.subscribe((result) => {
      if (result && result?.success) {
        ...
        this.complaintSearchRequest.satisfiedComplaint = result.value?.satisfiedComplaint;
        this.complaintSearchRequest.startCreatedAt = result.value
          ?.startCreatedAt
          ? CommonUtil.getStartOfDay((result.value?.startCreatedAt as Date).getTime())
          : undefined;
        this.complaintSearchRequest.endCreatedAt = result.value?.endCreatedAt
          ? CommonUtil.getEndOfDay((result.value?.endCreatedAt as Date).getTime())
          : undefined;
