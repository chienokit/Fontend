...
export class ComplaintCloseComponent implements OnInit {
  ...
  private initForm(): void {
    this.form = this.formBuilder.group({
      note: [null, [Validators.maxLength(this.MAX_DESCRIPTION_LENGTH), Validators.pattern(VALIDATORS.SPACE)]],
    });
  }
  ...
  closeComplaint(): void {
    if(this.complaint?.id) {
      this.complaintService.close(this.complaint?.id, this.form.controls.note.value, true)
      .subscribe((res: any)=> {
