...
import { LANGUAGE_EN, LANGUAGE_VI } from '@shared/constants/base.constant';
import { COMPLAINT_STATUS, COMPLAINT_TYPE } from '@shared/constants/complaint.constant';
...
import { ComplaintService } from '@shared/service/complaint.service';
...
export class MarkSpamsComponent implements OnInit {
  ...
  private initForm(): void {
    this.form = this.formBuilder.group({
      reason: [null, [Validators.required, Validators.maxLength(this.MAX_DESCRIPTION_LENGTH), Validators.pattern(VALIDATORS.SPACE)]],
      setOfCheckedId: [this.setOfCheckedId, [Validators.required, Validators.minLength(1)]],
    });
  }
  ...
  refreshCheckedStatus(complaints: IComplaint[]): void {
    ...
    if (this.currentUser?.userLevel === USER_LEVEL_LEADER_MANAGEMENT) {
      listOfEnabledData = this.complaints
        .filter((complaint) => {
          return (
            complaint.status === this.complaintStatus.OPEN.value &&
            complaint.complaintType === this.complaintType.UNDEFINED.value
  ...
  action(isAction: string): void {
    if (isAction === this.DELETE) {
      this.complaintService.deletes([...this.setOfCheckedId], true).subscribe((res: any) => {
        if (!!res && res?.body?.success) {
          this.toast.success('model.complaint.success.delete');
        }
      });
    } else if (isAction === this.CONFIRM) {
      this.complaintService.markAsSpam([...this.setOfCheckedId], this.form.controls.reason.value, true).subscribe((res: any) => {
        if (!!res && res?.body?.success) {
