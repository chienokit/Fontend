...
import {COMPLAINT_TYPE} from '@shared/constants/complaint.constant';
...
export class HandleComplaintComponent implements OnInit {
  ...
  initForm(): void {
    this.form = this.fb.group({
      feedback: [this.complaint?.feedback ? this.complaint?.feedback : null,
  ...
  onSubmit(): void {
    ...
    if (this.complaint.id) {
      this.complaintService.markAsMisClassification(this.complaint?.id, this.form.get('feedback')?.value, true).subscribe(res => {
