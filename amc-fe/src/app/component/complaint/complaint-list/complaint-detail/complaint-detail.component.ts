...
import { COMPLAINT_RATING, COMPLAINT_STATUS, COMPLAINT_TYPE } from '@shared/constants/complaint.constant';
...
export class ComplaintDetailComponent implements OnInit {
  ...
  public getRating(rating?: boolean): string {
    ...
    if (rating === this.complaintRating.UNSATIFIED.value) {
      return this.translateService.instant(this.complaintRating.UNSATIFIED.label);
    }
    return '';
  }
  ...
  getDetail(): void {
    this.complaintService.findById(this.complaintId).subscribe(res => {
      this.complaint = res.body?.data as IComplaint;
      if(this.complaint.complaintType == this.complaintType.UNDEFINED.value && this.complaint.status === this.complaintStatus.OPEN.value) {
      ...
      if (this.complaint?.ticketId) {
        this.ticketService.getById(this.complaint?.ticketId, true)
          .subscribe(resp => {
  ...
  handleAction(result: { success: boolean }): void {
    if (result && result?.success) {
      if(this.isConfirmCreateTicket && this.complaint.id) {
        this.route.navigate([`/ticket/create-by-complaint/${this.complaint.id}`]);
      }
