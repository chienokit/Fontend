...
import {ComplaintTemplateFilterComponent} from '@component/complaint/qr-manager/complaint-template-filter/complaint-template-filter.component';
...
export class QrManagerComponent implements OnInit {
  ...
  public searchQrCode(): void {
    this.qrTemplateService.searchComplaintTemplate(this.complaintSearchRequest).subscribe(
      res => {
  ...
  public getIndex(index: number): number {
    return CommonUtil.getIndex(index, this.complaintSearchRequest.pageIndex, this.complaintSearchRequest.pageSize);
  }
  ...
  public showLockConfirm(item: IComplaintTemplate): void {
    const base = CommonUtil.modalBase(HandleConfirmComponent, {
      complaintItem: item
    }, '50%');
  ...
  public unlockQrCode(complaintId: string, reason: string): void {
    ...
    this.qrTemplateService.active(complaintId, reason).subscribe(
      res => {
        this.searchQrCode();
        this.toastService.success('model.qr-manager.message.unlock-template-success');
      }
    );
  }
  ...
  public lockQrCode(complaintId: string, reason: string): void {
    ...
    this.qrTemplateService.inactive(complaintId, reason, true).subscribe(
      res => {
        this.searchQrCode();
        this.toastService.success(this.translateService.instant('model.qr-manager.message.lock-template-success'));
      }
    );
  }

  public filter(): void {
    const base = CommonUtil.modalBase(ComplaintTemplateFilterComponent, {
      filterRequest: this.complaintSearchRequest
    }, '50%');
    const modal: NzModalRef = this.modalService.create(base);
    modal.afterClose.subscribe(result => {
      if (result && result?.success) {
        this.complaintSearchRequest = {...this.complaintSearchRequest, ...result?.value};
