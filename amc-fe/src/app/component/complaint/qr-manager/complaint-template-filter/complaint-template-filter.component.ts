...
export class ComplaintTemplateFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.filterRequest?.createdUserIds && this.filterRequest?.createdUserIds?.length > 0) {
  ...
  onSearch(): void {
    if (this.form?.get('startAt')?.value) {
      this.form?.get('startAt')?.setValue(CommonUtil.setStartTime(new Date(this.form?.get('startAt')?.value).getTime()));
    }
    if (this.form?.get('endAt')?.value) {
      this.form?.get('endAt')?.setValue(CommonUtil.setEndTime(new Date(this.form?.get('endAt')?.value).getTime()));
    }
  ...
  changeStartAt(): void {
    this.startAt = moment(this.form?.get('startAt')?.value, 'DD/MM/yyyy').toDate();
    if (differenceInCalendarDays(this.form.get('endAt')?.value,
      this.form.get('startAt')?.value) < 0) {
  ...
  public onSearchBuilding(keyword?: string): void {
    ...
    this.buildingService.searchBuildingAutoComplete({keyword, sortBy: 'code.asc', buildingIds}).subscribe(res => {
  ...
  public onSearchFloor(keyword?: string): void {
    ...
    if (buildingIds && buildingIds?.length === 1) {
      ...
      // });
      this.buildingService.simpleSearchFloor(buildingIds[0], {keyword, sortBy: 'floorNumber.asc'}).subscribe(res => {
        this.floors = res.body?.data as Array<IFloor>;
      });
    }
  ...
  public onChangeBuilding(buildingIds?: string[]): void {
    ...
    if (buildingIds?.length === 1) {
      ...
      // load floor
      this.buildingService.simpleSearchFloor(buildingIds[0], {sortBy: 'floorNumber.asc'})
        .subscribe(res => {
          this.floors = res.body?.data as Array<IFloor>;
        });
    } else {
