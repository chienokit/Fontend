...
import {QR_STATUS, TEMPLATE_CHANGE_ACTION} from '@shared/constants/complaint.constant';
...
import {QrTemplateService} from '@shared/service/qr-template.service';
...
import {HandleConfirmComponent} from '@component/complaint/qr-manager/handle-confirm/handle-confirm.component';
import {NzModalRef, NzModalService} from 'ng-zorro-antd/modal';
import {ScriptPdfService} from '@shared/service/script-pdf.service';
...
export class QrUpdateComponent implements OnInit {
  ...
  public histories: Array<IComplaintTemplateHistory> = new Array<IComplaintTemplateHistory>();
  ...
  private initForm(): void {
    this.qrForm = this.fb.group({
      title: ['', [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.TITLE_MAX_LENGTH.MAX)]],
      ...
      buildingId: [{value: this.qrCodeDetail?.buildingId || '', disabled: this.isUpdate}, [Validators.required]],
    });
  ...
  searchFloorAutoComplete(keyword?: string): void {
    ...
    if (buildingId) {
      this.buildingService.simpleSearchFloor(buildingId, {keyword, sortBy: 'floorNumber.asc'}).subscribe(res => {
  ...
  private loadChangeHistories(): void {
    if (this.qrTemplateId) {
      this.qrTemplateService.getHistoriesComplaintTemplate(this.qrTemplateId, this.historiesSearchRequest).subscribe(res => {
  ...
  public onSubmit(): void {
    if (this.qrForm?.valid) {
      ...
      this.qrTemplateService.create(qrTemplate).subscribe(res => {
        ...
        if (template) {
          this.router.navigate([`${ROUTER_UTILS.complaint.root}/${template.id}/qr-update`]);
        } else {
          this.router.navigate([`${ROUTER_UTILS.complaint.root}/${ROUTER_UTILS.complaint.qrList}`]);
        }
        this.toast.success(this.translateService.instant('model.qr-manager.message.create-success'));
      });
    }
  }
  ...
  public getCurrentBuilding(): IBuilding | undefined {
    return this.buildings.find(building => building.id === this.qrForm.get('buildingId')?.value);
  }
  ...
  public getIndex(index: number): number {
    return CommonUtil.getIndex(index, this.historiesSearchRequest.pageIndex, this.historiesSearchRequest.pageSize);
  }
  ...
  getActionHistory(action?: string): string {
    return this.historyAction.find(item => item.value === action)?.label || '-';
  }
  ...
  public showUpdateConfirm(): void {
    ...
    modal.afterClose.subscribe(result => {
      if (result && result?.success && !!this.qrTemplateId) {
        ...
        this.qrTemplateService.update(this.qrTemplateId, qrTemplate, true).subscribe(() => {
          this.toast.success(this.translateService.instant('model.qr-manager.message.update-success'));
          this.loadChangeHistories();
        });
      }
    });
  ...
  generatePdf(action = 'open'): void {
    const floorName = this.floors.find(f => f.id === this.qrCodeDetail?.floorId)?.name || 'floorName';
    const buildingName = this.buildings.find(b => b.id === this.qrCodeDetail?.buildingId)?.name || 'buildingName';
    pdfMake.font = {
      Roboto: {
    ...
    const documentDefinition = this.getDocumentDefinition(floorName,
      buildingName);
    switch (action) {
      case 'open':
      ...
      case 'print':
        ...
        break;
      case 'download':
        pdfMake.createPdf(documentDefinition).download(`${floorName}_${buildingName}.pdf`);
  ...
  getDocumentDefinition(floorName: string, buildingName: string): any {
    ...
    return {
      watermark: {text: 'MB-AMC', angle: 45, opacity: 0.1, bold: true, italics: true},
      info: {
