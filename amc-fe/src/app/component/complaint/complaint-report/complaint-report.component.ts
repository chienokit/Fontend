...
} from '@component/complaint/complaint-report/complaint-report-filter/complaint-report-filter.component';
...
export class ComplaintReportComponent implements OnInit {
      ...
      .subscribe((res) => {
        if (res.body?.data) {
          ...
          if (!!buildings) {
            ...
            this.statisticRequest.startAt = this.startDate ? CommonUtil.getStartOfDay(this.startDate.getTime()) : undefined;
            this.statisticRequest.endAt = this.endDate ? CommonUtil.getEndOfDay(this.endDate.getTime()) : undefined;
            this.loadDataChart();
          }
        }
      });
  ...
  loadDataChart(): void {
    this.complaintService.statisticComplaint(this.statisticRequest, true).subscribe((res) => {
      ...
      this.buildings.forEach((item) => {
        ...
        if (dataChart) {
          ...
          item.classifySatisfiedChart = this.initClassifySatisfiedChart(dataChart);
          item.needToImproveChart = this.initNeedToImproveChart(dataChart);
          item.satisfiedStatusChart = this.initSatisfiedStatusChart(dataChart);
        }
      });
    });
  }
