...
export class NotificationUpdateComponent implements OnInit, AfterViewInit {
  ...
  onSubmit(): void {
    ...
    if (this.filesUpload?.length > 0) {
      ...
      this.fileService.uploadListFile(originFiles, '', OWNER_TYPE.NOTIFICATION).subscribe(res => {
  ...
  onUpdateSubmit(): void {
    ...
    if (this.filesUpload?.length > 0) {
      ...
      this.fileService.uploadListFile(originFiles, '', OWNER_TYPE.NOTIFICATION).subscribe(res => {
        const ids = res.body?.data as any;
        const fileUploadIds = ids.map((item: { id: string }) => item.id) as [];
  ...
  update(body: object): void {
    if (this.notification?.id) {
      this.notificationService.update(body, this.notification?.id).subscribe(res => {
        if (res.status) {
          const navigationExtras: NavigationExtras = {
            state: {
              tabIndex: this.isInternal ? INTERNAL : CUSTOMER,
  ...
  onChangeBuilding(): void {
    ...
    if (buildingIds) {
      const isEqual = CommonUtil.compareArrayIsEqual(this.buildingsPrevious, buildingIds);
      ...
      if (!isEqual && !this.isInternal) {
        if (buildingIds.length === 1) {
          ...
          this.buildingService.searchFloorByBuilding(buildingIds[0], request).subscribe(res => {
  ...
  getFormValue(): any {
    ...
    return {
      ...
      note: data?.note,
      expectedNotificationAt: CommonUtil.getEndOfDay(new Date(data?.date).getTime()),
      fileIds: [],
      eventTargetType: this.isInternal ? INTERNAL : CUSTOMER,
      eventType: ((this.currentUser?.userLevel === USER_LEVEL_CENTER) && !this.isInternal) ? 'NOTIFICATION_EMAIL' : 'NOTIFICATION',
      eventLevel: this.currentUser?.userLevel === USER_LEVEL_CENTER ? 'CENTER' : 'MANAGER'
    };
  }
  ...
  initFormGroup(notification?: INotification, isUpdate = false, isDetail = false): void {
    this.form = this.fb.group({
      buildingIds: [
        {
          value: isUpdate ? this.mappingBuildings(notification?.buildings || []) : [],
          disabled: isDetail
        },
        [
          Validators.required
      ...
      floorIds: [
        {value: isUpdate ? this.mappingFloors(notification?.floors || []) : [], disabled: isDetail},
        [
          // Validators.required
        ]
      ],
      customerIds: [
        {
          value: isUpdate ? this.mappingOrganizations(notification?.organizations || []) : [],
          disabled: isDetail
        },
        [
          // Validators.required
      ...
      content: [
        {value: isUpdate ? this.notification?.content : null, disabled: isDetail},
        [
          Validators.required
        ]
      ],
      ...
      date: [
        {
          value: isUpdate ? new Date(this.notification?.expectedNotificationAt) : new Date(),
          disabled: isDetail
        },
        [
          Validators.required
  ...
  getFloorsByBuildings(buildingId: string): void {
    ...
    this.buildingService.searchFloorByBuilding(buildingId, request, true).subscribe(res => {
      this.floors = res.body?.data as Array<Floor>;
    });
  }
  ...
  getNotificationById(notificationId: string): void {
    this.notificationService.find(notificationId, true).subscribe(res => {
      if (res.status === STATUS.SUCCESS_200) {
        ...
        this.initFormGroup(res?.body?.data as Notification, this.isUpdate, this.isDetail);
        if (this.isInternal) {
          if (this.notification?.buildings && this.notification?.buildings?.length > 0) {
            this.getBuildings();
            const buildingIds = this.mappingBuildings(this.notification?.buildings || []);
          ...
          if (this.notification?.buildings && this.notification?.buildings?.length === 1) {
            this.getBuildings();
            const buildingIds = this.mappingBuildings(this.notification?.buildings);
            this.getFloorsByBuildings(buildingIds[0] || '');
            if (!this.notification?.floors && !this.notification?.organizations) {
              this.getCustomersByBuildings(buildingIds);
            } else if (this.notification?.floors && this.notification?.floors.length > 0) {
              const floorIds = this.mappingFloors(this.notification?.floors);
              this.getCustomersByFloors({ids: floorIds});
            }
          } else if (this.notification?.buildings && this.notification?.buildings?.length > 1) {
            this.getBuildings();
            this.floors = [];
            const buildingIds = this.mappingBuildings(this.notification?.buildings);
            this.getCustomersByBuildings(buildingIds);
