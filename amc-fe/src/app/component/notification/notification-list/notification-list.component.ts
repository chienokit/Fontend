...
export class NotificationListComponent implements OnInit {
  ...
  detail(notification: Notification): void {
    if (this.userLevel.USER_LEVEL_CENTER) {
      ...
      this.router.navigate([`/notification/${notification?.id}/detail/${type.toLowerCase()}`]);
    } else {
      this.router.navigate([`/notification/${notification?.id}/detail/${CUSTOMER.toLowerCase()}`]);
    }
  ...
  update(notification: Notification): void {
    if (this.userLevel.USER_LEVEL_CENTER) {
      const type = this.tabIndex === this.tabInternal ? INTERNAL : CUSTOMER;
      this.router.navigate([`/notification/${notification?.id}/update/${type.toLowerCase()}`]);
    } else {
      this.router.navigate([`/notification/${notification?.id}/update/${CUSTOMER.toLowerCase()}`]);
    }
  }
  ...
  onDelete(result: { success: boolean }): void {
    if (result && result?.success) {
      this.notificationService.delete(this.notification.id, true).subscribe(res => {
  ...
  onSend(result: { success: boolean }): void {
    if (result && result?.success) {
      this.notificationService.send(this.notification.id, true).subscribe(res => {
        if (res.status === STATUS.SUCCESS_200) {
          if (this.userLevel.USER_LEVEL_CENTER) {
            if (this.tabIndex === this.tabInternal) {
              this.loadData(this.querySearchInternal);
  ...
  loadData(request: any, isLoading = true): void {
    ...
    this.notificationService.search(CommonUtil.formatParams(param), isLoading).subscribe(response => {
  ...
  getIndex(index: number): number {
    if (this.userLevel.USER_LEVEL_CENTER) {
      if (this.tabIndex === this.tabInternal) {
        return CommonUtil.getIndex(index, this.querySearchInternal.pageIndex, this.querySearchInternal.pageSize);
      } else {
        return CommonUtil.getIndex(index, this.querySearchCustomer.pageIndex, this.querySearchCustomer.pageSize);
      }
    } else {
      return CommonUtil.getIndex(index, this.querySearchCustomer.pageIndex, this.querySearchCustomer.pageSize);
    }
  ...
  openModalViewFile(item: Notification): void {
    const base = CommonUtil.modalBase(ModalFileComponent, {
  ...
  filter(): void {
    ...
    if (this.userLevel.USER_LEVEL_CENTER) {
      ...
      filter = type === INTERNAL ? {...this.filterModalInternal} : {...this.filterModalCustomer};
    } else {
      type = CUSTOMER;
      filter = {...this.filterModalCustomer};
    }
    const base = CommonUtil.modalBase(NotificationFilterComponent, {
      ...filter,
    ...
    modal.afterClose.subscribe(result => {
      if (result && result?.success) {
        ...
        const request = {
          ...
          expectedStartDate: result?.value.startDate ? CommonUtil.getStartOfDay(startDate) : null,
          expectedEndDate: result?.value.endDate ? CommonUtil.getEndOfDay(endDate) : null,
          notificationStartAt: result?.value.startDate ? CommonUtil.getStartOfDay(startDate) : null,
          notificationEndAt: result?.value.endDate ? CommonUtil.getEndOfDay(endDate) : null,
