...
export class NotificationFilterComponent implements OnInit {
  ...
  getExitedData(): void {
    if (this.form.get('buildingIds')?.value) {
      ...
      this.buildingService.searchFloorByBuilding(buildingId, {}, true)
      .subscribe((response: any) => {
        this.floors = response?.body?.data;
      });
      this.customerService.findCustomers({ids: buildingId})
    .subscribe((res: any) => {
  ...
  enterStartDatePicker(event: any): void {
    ...
    if (CommonUtil.newDate(date).toString() === 'Invalid Date') {
      this.form.controls.startDate?.setValue(this.form.controls.startDate?.value);
    ...
    this.form?.controls.buildingIds?.valueChanges.subscribe(value => {
      if (value && value.length === 0) {
        ...
        if (this.type === CUSTOMER) {
          ...
          this.buildingService.searchFloorByBuilding(buildingId, request, true)
            .subscribe((response: any) => {
              this.floors = response?.body?.data;
            });
          this.customerService.findCustomers({ids: value})
            .subscribe((res: any) => {
              this.organizations = res?.body?.data;
            });
          this.form?.get('floorIds')?.setValue(null);
        ...
        if (value) {
          if (value && value.length > 0) {
            ...
            if (buildingIds.length > 0) {
              this.customerService.findCustomers({ids: buildingIds}).subscribe((res: any) => {
                this.organizations = res?.body?.data;
                this.form?.get('organizationIds')?.setValue(null);
              });
            }
