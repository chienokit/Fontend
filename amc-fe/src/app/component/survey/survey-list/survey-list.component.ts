...
export class SurveyListComponent implements OnInit {
  ...
  loadData(page: number, size: number, isLoading = true): void {
    ...
    if (!!query.startAt) {
      query.startAt = CommonUtil.getStartOfDay(this.surveyRequest.startAt || 0) || undefined;
    }
    if (!!query.endAt) {
      query.endAt = CommonUtil.getEndOfDay(this.surveyRequest.endAt || 0) || undefined;
    }
    this.surveyService.search(query, isLoading).subscribe((response: any) => {
  ...
  actionCustom(result: { success: boolean }): void {
    ...
    if (this.action === this.ACTION_LIST.delete) {
      ...
      this.surveyService.sendSurvey(this.survey?.id || '', true).subscribe(response => {
  ...
  openAdvancedSearch(): void {
    const base = CommonUtil.modalBase(SurveyAdvanceSearchComponent, this.surveyRequest, '30%');
