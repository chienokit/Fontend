...
import {ISurveyTemplate, SurveyTemplate} from '@shared/models/survey-template.model';
...
import {QuestionGroupService} from '@shared/service/question-group.service';
import {SurveyTemplateService} from '@shared/service/survey-template.service';
...
export class UpdateSurveyComponent implements OnInit {
  ...
  initForm(): void {
    this.form = this.fb.group({
      name: [{value: (this.isUpdate || this.isDetail) ? this.survey?.name : null, disabled: this.isDetail},
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.NAME_MAX_LENGTH.MAX)]],
      introduction: [{
        value: (this.isUpdate || this.isDetail) ? this.survey?.introduction : null,
        disabled: this.isDetail
      },
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.CONTENT_MAX_LENGTH.MAX)]],
      notificationContent: [{
        value: (this.isUpdate || this.isDetail) ? this.survey?.notificationContent : null,
        disabled: this.isDetail
      },
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.CONTENT_MAX_LENGTH.MAX)]],
      otherOpinion: [{
        value: (this.isUpdate || this.isDetail) ? this.survey?.surveyTemplate?.otherOpinion : '',
        disabled: this.isDetail
      },
        [Validators.required, Validators.maxLength(LENGTH_VALIDATOR.OPINION_MAX_LENGTH.MAX)]],
      note: [{value: (this.isUpdate || this.isDetail) ? this.survey?.note : '', disabled: this.isDetail},
      ...
      organizationIds: [{
        value: (this.isUpdate || this.isDetail) ? this.survey?.organizationIds : [],
        disabled: this.isDetail || !(this.isUpdate || this.isDetail)
      }],
      startAt: [{
        value: (this.isUpdate || this.isDetail) ? new Date(this?.survey?.startAt || 0) : new Date(),
        disabled: this.isDetail
      },
        [Validators.required]],
      endAt: [{
        value: (this.isUpdate || this.isDetail) ? new Date(this?.survey?.endAt || 0) : new Date(),
        disabled: this.isDetail
      },
        [Validators.required]],
      surveyTemplateId: [{
        value: (this.isUpdate || this.isDetail) ? this.survey?.surveyTemplateId : null,
        disabled: this.isDetail
      },
        [Validators.maxLength(LENGTH_VALIDATOR.ID_MAX_LENGTH.MAX), Validators.required]],
      surveyTemplateName: [{value: '', disabled: this.isDetail},
  ...
  initData(): void {
    ...
    // });
    this.surveyTemplateService.search({sortBy: 'name.asc'}, true)
  ...
  onUpdateSubmit(): void {
    ...
    this.surveyService.update(survey, this.survey?.id, true).subscribe(res => {
  ...
  handleMapSurvey(form: FormGroup): Survey {
    ...
    survey.startAt = CommonUtil.getStartOfDay(new Date(form?.get('startAt')?.value).getTime());
    survey.endAt = CommonUtil.getEndOfDay(new Date(form?.get('endAt')?.value).getTime());
  ...
  initSurvey(id: string): void {
    this.surveyService.findBySurveyId(id, true).subscribe((response: any) => {
      ...
      this.customerService.findIds({ids: response.body?.data?.organizationIds}, true).subscribe((res: any) => {
  ...
  handleMapFormToSurvey(survey: ISurvey): void {
    ...
    this.form.get('otherOpinion')?.setValue(survey?.surveyTemplate?.otherOpinion || null);
    ...
    this.form.get('surveyTemplateId')?.setValue(survey?.surveyTemplateId || null);
    this.form.get('surveyTemplateName')?.setValue('');
    this.form.get('notificationContent')?.setValue(survey?.notificationContent || null);
  ...
  searchFloorByBuilding(event: any): void {
    ...
    this.buildingService.searchFloorByBuilding(this.form?.get('buildingIds')?.value, option, true)
      .subscribe((response: any) => {
        this.floors = response?.body?.data;
      });
  }
  ...
  createNewSurveyTemplate(): void {
    ...
    if (this.isCreateSurveyTemplate) {
      this.form?.controls?.surveyTemplateName?.setValidators(Validators.required);
  ...
  createSurveyTemplate(surveyTemplate: ISurveyTemplate): void {
    if (!!surveyTemplate.id) {
      ...
      for (const key of (surveyTemplate?.questionGroups || [])) {
        if (!!key) {
          this.criteriaQuestions.push(new Criteria(key.name, key.label, key.id, key.questions));
  ...
  searchSurveyTemplate(event: any): void {
    ...
    this.keySurveyTemplate = keyword?.trim();
    this.surveyTemplateService.search(option, true)
      .subscribe((response: any) => {
        this.surveyTemplates = response?.body?.data;
      });
  ...
  changeSurveyTemplate(surveyTemplateId: any): void {
    if (!!surveyTemplateId) {
      this.surveyTemplateService.getById(surveyTemplateId, true).subscribe((response: any) => {
        if (response?.body?.success) {
          this.clearSurveyTemplate();
          this.surveyTemplate = response?.body?.data;
          this.form?.get('surveyTemplateName')?.setValue(this.surveyTemplate?.name);
          if (this.surveyTemplates?.filter(item => item?.id === this.surveyTemplate?.id).length <= 0) {
...
//  criteria
  ...
  addItemCriteriaControl(): void {
    (this.form.get('criteria') as FormArray).push(this.createItemCriteriaControl(new Criteria()));
...
//  Questions
  ...
  addItemControl(index: number): void {
    (this.criteria()?.at(index)?.get('questions') as FormArray).push(this.createItemControl(new Question()));
  ...
  removeItemControl(idxCriteria: number, index: number): void {
    ...
    // }
    (this.criteria().at(idxCriteria).get('questions') as FormArray).removeAt(index);
    this.checkQuestions = !(this.criteria().at(idxCriteria).get('questions') as FormArray).length;
  }
  ...
  getQuestionInGroup(idx: number): string {
    ...
    this.questions(idx).controls.forEach((item, i) => {
      content += `<p class="question m-0"><b class="p-3"><u class="pb-2">${this.getTranslate('question.title')}
  ...
  disabledBeforeStartAt(current: Date): boolean {
    ...
    return differenceInCalendarDays(current, moment(date?.value, 'DD/MM/yyyy').toDate()) < 0;
  }
  ...
  changeStartAt(): void {
    this.startAt = moment(this.form?.controls?.startAt?.value, 'DD/MM/yyyy')?.toDate();
    if (this.startAt?.toString() !== 'Invalid Date') {
      if (differenceInCalendarDays(this.form.controls.endAt?.value,
        this.form.controls.startAt?.value) < 0) {
  ...
  valueChangesForm(): void {
    this.form?.controls.buildingIds?.valueChanges?.subscribe(value => {
      ...
      // this.form.get('organizationIds')?.reset();
      if (value?.length === 1) {
        this.buildingService.searchFloorByBuilding(this.form?.get('buildingIds')?.value, {sortBy: 'name.asc'}, true)
          .subscribe((response: any) => {
            this.floors = response?.body?.data;
            floorTmp = floorTmp?.filter((item: string) => {
              return response?.body?.data?.map((floor: IFloor) => {
                return floor?.id;
              })?.includes(item);
            });
    ...
    this.form?.controls.floorIds?.valueChanges?.subscribe(value => {
      ...
      // this.form.get('organizationIds')?.reset();
      if (value?.length > 0) {
        this.customerService.findFloors({ids: value}).subscribe((res: any) => {
          this.organizations = res?.body?.data;
          orgTmp = orgTmp?.filter((item: string) => {
            return res?.body?.data?.map((org: ICustomer) => {
              return org?.id;
        ...
        this.customerService.findCustomers({ids: buildingIds}, true).subscribe((res: any) => {
          this.organizations = res?.body?.data;
          orgTmp = orgTmp?.filter((item: string) => {
            return res?.body?.data?.map((org: ICustomer) => {
              return org?.id;
