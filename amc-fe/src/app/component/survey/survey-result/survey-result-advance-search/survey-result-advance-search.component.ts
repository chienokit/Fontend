...
export class SurveyResultAdvanceSearchComponent implements OnInit {
  ...
  initData(): void {
    this.surveyResultRequest = this.modalRef?.getConfig()?.nzComponentParams || {};
    this.accountService.getBuildings().subscribe((res: any) => {
      this.buildings = res?.body?.data;
    });

    this.organizationService.search({sortBy: 'name.asc'}).subscribe((res: any) => {
      this.organizations = res?.body?.data;
      if (!!this?.surveyResultRequest?.organizationIds) {
        const ids = this?.surveyResultRequest?.organizationIds?.filter(item =>
          !(this.organizations?.map(organization => organization?.id)?.includes(item)));
        if (!(!!ids && ids?.length > 0)) {
          return;
        }
        this.organizationService.findIds({ids: ids || []}).subscribe((rs: any) => {
  ...
  valueChangesForm(): void {
    ...
    this.form?.controls.buildingIds?.valueChanges.subscribe(value => {
      ...
      if (!!value && value?.length > 0) {
        this.organizationService.findCustomers({ids: value}, true).subscribe((response: any) => {
