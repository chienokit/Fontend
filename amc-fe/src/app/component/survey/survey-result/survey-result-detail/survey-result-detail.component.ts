...
export class SurveyResultDetailComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.surveyService.searchDetailSummaryByCustomer(
      this.surveyId || '',
  ...
  loadDataCompany(buildingId: string, organizationId: string): void {
    this.surveyService.findDetailSummary(this.surveyId || '', {
      buildingId,
      organizationId,
    }).subscribe((response: any) => {
  ...
  detail(item: SurveyResult, screen: string): void {
    ...
    this.router.navigate([`/survey/result/${item.surveyId}/detail/${screen}`], navigationExtras);
  }
  ...
  cancel(): void {
    ...
    if (this.type === this.COMPANY && !this.beginStage) {
      this.router.navigate([`${this.SURVEY_RESULT_URL}`], {state: {tabIndex: this.tabIndex}});
    }
  }
