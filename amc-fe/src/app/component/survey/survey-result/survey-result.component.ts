...
export class SurveyResultComponent implements OnInit {
  ...
  option = {
    ...
    toolbox: {
      ...
      feature: {
        ...
        saveAsImage: {
          ...
          name: this.translateService.instant('model.survey.results.dashboardResult'),
        }
      }
    },
    legend: {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        return `<b>${params.seriesName}:</b> <br\> ${params.name}: ${Number(params.data[params.componentIndex + 1])
          .toFixed(2)}`;
      }
    },
    dataset: {
  ...
  constructor(
    ...
    const tabIndex = this.router?.getCurrentNavigation()?.extras?.state?.tabIndex;
    if (tabIndex) {
  ...
  getIndex(index: number): number {
    if (this.userLevel.USER_LEVEL_CENTER && this.tabIndex === this.tabStage) {
      return CommonUtil.getIndex(index, this.querySearchStage.pageIndex, this.querySearchStage.pageSize);
    }
    return CommonUtil.getIndex(index, this.querySearchCompany.pageIndex, this.querySearchCompany.pageSize);
  }
  ...
  onSearchSurveys(options: any): void {
    this.surveyService.searchSummaryBySurvey(options).subscribe((response: any) => {
  ...
  onSearchCompanies(options: any): void {
    this.surveyService.searchSummaryByCustomer(options).subscribe((response: any) => {
  ...
  export(item: SurveyResult, screen: string): void {
    if (screen === this.STAGE) {
      this.surveyService.export(item?.surveyId || '', {
        buildingId: item?.buildingId,
      }).subscribe(response => {
        CommonUtil.downloadFileType(response.body, response.body.type, `${item.survey?.name}-${item.building?.name}.xlsx`);
      });
    } else {
      this.surveyService.export(item?.surveyId || '', {
        buildingId: item?.buildingId,
        ...
        CommonUtil.downloadFileType(response.body, response.body.type, `${item.survey?.name}-${item.building?.name}-${item.organization?.name}.xlsx`);
      });
    }
  }
  ...
  detail(item: SurveyResult, screen: string): void {
    ...
    if (screen === this.STAGE) {
      this.router.navigate([`/survey/result/${item.surveyId}/detail/${screen}`]).then();
    } else if (screen === this.COMPANY) {
      this.router.navigate([`/survey/result/${item.surveyId}/detail/${screen}`], navigationExtras).then();
    }
  }
  ...
  initDataChart(surveyNames: Array<string>, results: IBuildingSurvey[]): void {
    ...
    if (results?.length <= 0) {
      if (this.form?.get('buildingIds')?.value?.length > 0) {
        results = this.buildingList?.filter(item =>
          (this.form.get('buildingIds')?.value)?.includes(item?.id))?.map(item =>
          new BuildingSurvey(item?.name, [0]));
      } else {
        results = this.buildingList?.map(item => new BuildingSurvey(item?.name, [0]));
      }
      surveyNames = [''];
    }
  ...
  onSearchChartSurvey(): void {
    ...
    if (!!param?.surveyIds && param?.surveyIds?.length > 0) {
      this.surveyService.getChartSurvey(param).subscribe((res: any) => {
        ...
        this.initDataChart(this.chartSurvey?.labels || [], this.chartSurvey?.data || []);
      });
    }
  }
  ...
  valueChangesForm(): void {
    this.form?.controls.surveyIds?.valueChanges.subscribe(value => {
      if (!!value) {
        this.surveyService.getBuildingsBySurveyIds({ids: this.form?.get('surveyIds')?.value})
          .subscribe((res: any) => {
            this.buildingList = res?.body?.data;
            this.form.get('buildingIds')?.reset();
          });

        this.questionGroupService.getBySurveyIds({ids: this.form?.get('surveyIds')?.value})
    ...
    this.form?.controls.questionGroupIds?.valueChanges.subscribe(() => {
      this.questionGroupService.getAllQuestionByQuestionGroupIds({ids: this.form?.get('questionGroupIds')?.value})
        .subscribe((response: any) => {
  ...
  openAdvancedSearch(): void {
    if (this.tabIndex === this.tabStage) {
      const base = CommonUtil.modalBase(SurveyResultAdvanceSearchComponent, this.querySearchStage, '40%');
      ...
      modal.afterClose.subscribe(res => {
        if (res && res?.success) {
          ...
          if (!!result?.averageScore) {
            this.querySearchStage.averageScoreMin = result?.averageScore.charAt(0);
            this.querySearchStage.averageScoreMax = result?.averageScore.charAt(2);
          } else {
          ...
          query.averageScore = '';
          if (!!result?.startAt) {
            query.startAt = CommonUtil.getStartOfDay((result?.startAt as Date)?.getTime()) + '';
          } else {
            query.startAt = '';
          }
          if (!!result?.endAt) {
            query.endAt = CommonUtil.getEndOfDay((result?.endAt as Date)?.getTime()) + '';
          } else {
            query.endAt = '';
          }
          this.loadData(query);
      ...
      const base = CommonUtil.modalBase(SurveyResultAdvanceSearchComponent, this.querySearchCompany, '40%');
      const modal: NzModalRef = this.modalService.create(base);
      modal.afterClose.subscribe(res => {
        if (res && res?.success) {
          const result = res?.value;
          ...
          this.querySearchCompany.averageScoreMin = result?.averageScore?.charAt(0);
          this.querySearchCompany.averageScoreMax = result?.averageScore?.charAt(2);
          this.querySearchStage.pageIndex = PAGINATION.PAGE_DEFAULT;
