...
import { SURVEY_FILENAME_TEMP, SURVEY_STATUS_DONE } from '@shared/constants/survey.constants';
...
export class SurveyResultExportComponent implements OnInit {
  ...
  getSurveys(): void {
    this.surveyService.searchAutoComplete(this.request, false).subscribe((res: any) => {
  ...
  export(): void {
    ...
    const surveyName = this.surveys.filter(survey => survey.id === surveyId)[0]?.name || SURVEY_FILENAME_TEMP;
    this.surveyService.exportAll(surveyId || '', true).subscribe(response => {
      CommonUtil.downloadFileType(response.body, response.body.type, `${surveyName}.xlsx`);
    });
    this.modalRef.close({
      success: true,
      value: {},
