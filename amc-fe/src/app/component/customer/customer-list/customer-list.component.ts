...
export class CustomerListComponent implements OnInit {
  ...
  loadData(page: number, size: number, isLoading = true): void {
    ...
    this.customerService.search(request, isLoading).subscribe((response: any) => {
  ...
  onLockAndUnLock(result: { success: boolean }): void {
    if (result && result?.success) {
      if (this.customer.status === STATUS_ACTIVE) {
        this.customerService.inactive(this.customer.id, true).subscribe(res => {
  ...
  filter(): void {
    const base = CommonUtil.modalBase(CustomerFilterComponent, {
      ...this.filterModal
    }, '30%');
    ...
    modal.afterClose.subscribe(result => {
      if (result && result?.success) {
        ...
        const incorporationDate = result?.value?.date ? moment(result?.value?.date).format('yyyy-MM-DD') : null;
        this.filterModal = {
          incorporationDate: this.filterModal.incorporationDate ? incorporationDate : null,
