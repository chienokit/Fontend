...
import {BUILDING_STATUS, LEASING_STATUS} from '@shared/constants/building.constants';
import {LEASED, LEASING_STATUS_CREATE} from '@shared/constants/customer.constants';
...
export class CustomerLeasingComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.isUpdate) {
      ...
      this.customerService.createUnit(this.organizationId, body, true).subscribe(res => {
  ...
  getFloors(buildingId: string): void {
    ...
    this.buildingService.simpleSearchFloor(buildingId, pagingRequest).subscribe(res => {
  ...
  private initForm(): void {
    this.form = this.fb.group({
      id: [
        {value: this.isUpdate ? this.leasing.id : null, disabled: false},
      ],
      buildingId: [
        {value: this.isUpdate ? this.leasing.buildingId : null, disabled: false},
        [
          Validators.required
        ]
      ],
      ...
      leasingStatus: [
        {value: this.isUpdate ? this.leasing.leasingStatus : LEASED, disabled: false},
        [
          Validators.required,
        ]
      ],
