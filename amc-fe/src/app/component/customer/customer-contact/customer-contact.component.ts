...
export class CustomerContactComponent implements OnInit {
  ...
  private initForm(): void {
    this.form = this.fb.group({
      id: [
        {value: this.isUpdate ? this.customerContact.id : null, disabled: false},
      ],
      dayOfBirth: [
        {
          value: this.isUpdate ? this.customerContact.dayOfBirth : moment(new Date()).format('yyyy-MM-DD'),
          disabled: false
        },
      ],
      gender: [
        {value: this.isUpdate ? this.customerContact.gender : GENDER.OTHER, disabled: false},
      ],
      fullName: [
        {value: this.isUpdate ? this.customerContact.fullName : null, disabled: false},
        [
          Validators.required
        ]
      ],
      email: [
        {value: this.isUpdate ? this.customerContact.email : null, disabled: false},
        [
          Validators.required, Validators.pattern(VALIDATORS.EMAIL)
        ]
      ],
      phoneNumber: [
        {value: this.isUpdate ? this.customerContact.phoneNumber : null, disabled: false},
        [
  ...
  onSubmit(): void {
    ...
    if (this.isUpdate) {
      this.customerService.updateContacts(this.organizationId, body.id, body, true).subscribe(res => {
      ...
      this.customerService.createContacts(this.organizationId, body, true).subscribe(res => {
        if (res.status === STATUS.SUCCESS_200) {
          this.toast.success('common.success');
          this.modalRef.close({
            success: true,
