...
import { CUSTOMER_TYPE, LEASED, LEASING_STATUS, RETURNED } from '@shared/constants/customer.constants';
import { PAGINATION } from '@shared/constants/pagination.constants';
import {COMMON_STATUS, STATUS, STATUS_ACTIVE} from '@shared/constants/status.constants';
...
import { CustomerContact, ICustomerContact } from '@shared/models/customer-contact.model';
...
export class CustomerUpdateComponent implements OnInit {
  ...
  initForm(): void {
    this.form = this.fb.group({
      type: [
        {value: this.isUpdate ? this.customer?.type : CUSTOMER_TYPE[0].value, disabled: false},
        [
          Validators.required
        ]
      ],
      code: [
        {value: this.isUpdate ? this.customer?.code : null, disabled: this.isUpdate},
        // [
        //   Validators.required,
      ...
      legalRepresentative: [
        {value: this.isUpdate ? this.customer?.legalRepresentative : null, disabled: false},
        [
          Validators.required,
          Validators.maxLength(LENGTH_VALIDATOR.NAME_MAX_LENGTH.MAX)
        ]
      ...
      status: [
        {value: this.isUpdate ? this.customer?.status : COMMON_STATUS[0].value, disabled: false},
        [
          Validators.required
        ]
      ],
      incorporationDate: [
        this.isUpdate ? (this.customer?.incorporationDate ? new Date(this.customer?.incorporationDate) : null) : null,
        [
          // Validators.required
        ]
      ],
  ...
  createInformationCustomer(): void {
    ...
    if (body?.incorporationDate) {
      body.incorporationDate = moment(body?.incorporationDate).format('yyyy-MM-DD');
    }
    this.customerService.findByBusinessCode(body.businessCode.trim(), true).subscribe(res => {
      if (res?.body?.code === STATUS.ERROR_404 || !res?.body?.success) {
        this.customerService.create(CommonUtil.trim(body), true).subscribe((response: any) => {
          if (response.status === STATUS.SUCCESS_200) {
            ...
            this.router.navigate([`/customer/${data.id}/update`], navigationExtras);
          }
        });
      } else {
  ...
  updateInformationCustomer(): void {
    ...
    this.customerService.update(CommonUtil.trim(body), this.customerId, true).subscribe(res => {
  ...
  onCreateContact(): void {
    const base = CommonUtil.modalBase(CustomerContactComponent, {
      isUpdate: false,
  ...
  onCreateLeasing(): void {
    const base = CommonUtil.modalBase(CustomerLeasingComponent, {
      isUpdate: false,
      organizationId: this.customerId,
    }, '45%');
    const modal: NzModalRef = this.modalService.create(base);
  ...
  searchContacts(customerId: any, request: object, isLoading = false): void {
    this.customerService.searchContacts(customerId, request, isLoading).subscribe((response: any) => {
  ...
  searchLeasing(customerId: any, request: object, isLoading = false): void {
    this.customerService.searchLeasing(customerId, request, isLoading).subscribe((response: any) => {
  ...
  format(value: any, type: string): any {
    if (type === 'leasing') {
      const leasingStatus = this.leasingStatus
        .filter((item: { label: string, value: string }) => item.value === value);
      if (leasingStatus.length > 0) {
          const translate = leasingStatus[0].label;
          return this.translateService.instant(translate);
        }
    } else if (type === 'customer') {
      const translate = this.customerType
  ...
  onDeleteContact(result: { success: boolean }): void {
    if (result && result?.success) {
      this.customerService.deleteContact(this.customerId, this.itemDelete.id, true).subscribe(res => {
    ...
    if (result && result?.success) {
      switch (this.groupLeasingPopup.type) {
        case 'delete': {
          ...
          this.customerService.deleteUnit(this.customerId, unitId, true).subscribe(res => {
        ...
        case 'paid': {
          ...
          this.customerService.leaseUnit(this.customerId, unitId, true).subscribe(res => {
        ...
        case 'returned': {
          ...
          this.customerService.returnUnit(this.customerId, unitId, true).subscribe(res => {
  ...
  enterDatePicker(event: any): void {
    ...
    if (CommonUtil.newDate(date).toString() === 'Invalid Date') {
      this.form.controls.incorporationDate.setValue(this.form.controls.incorporationDate.value);
  ...
  search(event: any): void {
    ...
    if (code.length > 0) {
      this.customerService.findByBusinessCode(code.trim(), true).subscribe((res: any) => {
        if (res?.body?.code === STATUS.ERROR_404 || !res?.body?.success) {
