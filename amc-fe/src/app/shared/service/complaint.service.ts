...
export class ComplaintService extends AbstractService {
  ...
  public findById(id: string, loading = false): Observable<EntityResponseType<IComplaint>> {
  ...
  public delete(id: string, loading = false): Observable<EntityResponseType<IComplaint>> {
    return super.post<IComplaint>(`${this.resourceUrl}/${id}/delete`, {}, {loading});
  }

  public deletes(complaintIds: string[], loading = false): Observable<EntityResponseType<IComplaint>> {
    return super.post<IComplaint>(`${this.resourceUrl}/delete`, {complaintIds}, {loading});
  }

  public markAsSpam(complaintIds: string[], invalidReason: string, loading = false): Observable<EntityResponseType<boolean>> {
    return super.post<boolean>(`${this.resourceUrl}/mark-as-spam`, {complaintIds, invalidReason}, {loading});
  }

  public close(complaintId: string, note: string, loading = false): Observable<EntityResponseType<boolean>> {
    return super.post<boolean>(`${this.resourceUrl}/close `, {complaintId, note}, {loading});
  }

  public markAsMisClassification(complaintId: string, feedback?: string, loading = false): Observable<EntityResponseType<IComplaint>> {
    return super.post<IComplaint>(`${this.resourceUrl}/${complaintId}/mark-as-mis-classification`, {feedback}, {loading});
  }
  ...
  public searchComplaints(params?: any, loading = false): Observable<EntityResponseType<IComplaint[]>> {
    return super.get<IComplaint[]>(this.resourceUrl, {loading, params});
  }

  public statisticComplaint(params: IComplaintStatisticRequest, loading = false): Observable<EntityResponseType<IFeedBackIStatistical[]>> {
    return super.post<IFeedBackIStatistical[]>(`${this.resourceUrl}/statistic`, params, {loading});
  }

  createComplaint(body: Complaint, loading = false): Observable<EntityResponseType<IComplaint>> {
    return super
      .post<IComplaint>(`${this.publicUrl}/public/complaints`, body, {loading});
  }
