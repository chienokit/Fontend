...
export class QuestionGroupService extends AbstractService {
  ...
  getById(questionGroupId: string, loading = false): Observable<EntityResponseType<ICriteria>> {
    return super.get<ICriteria>(`${this.resourceUrl}/${questionGroupId}`, {loading});
  }
  ...
  getAllQuestionByQuestionGroupIds(options: {}, loading = false): Observable<EntityResponseType<IQuestion[]>> {
    return super.post<IQuestion[]>(`${this.resourceUrl}/questions`, options, {loading});
  }

  getBySurveyIds(options: {}, loading = false): Observable<EntityResponseType<ICriteria[]>> {
    return super.post<ICriteria[]>(`${this.resourceUrl}/find-by-survey-ids`, options, {loading});
  }
}
