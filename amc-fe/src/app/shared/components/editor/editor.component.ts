...
import { MAX_LENGTH_EDITOR } from '../../constants/common.constant';
...
export class EditorComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  private static generateId(): string {
    return 'editor' + Math.ceil(Math.random() * 100) + Math.ceil(Math.random() * 100);
  }
  ...
  async afterView(): Promise<void> {
    setTimeout(async () => {
      ...
      if (document.querySelector(`#${this.idEditor}`) && !this.isLoadCKeditor) {
        ...
        await ClassicEditor.create(document.querySelector(`#${this.idEditor}`), {
          toolbar: {
            items: [
              'heading',
              'bold',
